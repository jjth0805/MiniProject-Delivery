...
<script>
    function delConfirm() {
        if(confirm("삭제 하시겠습니까?")){
            location.href = "<c:url value='/board/boardDelete?boardNum=${vo.boardNum}'/>";
            }
    }
    function updateConfirm(){
        if(confirm("수정 화면으로 이동하시겠습니까?")){
            location.href = "<c:url value='/board/boardUpdateForm?boardNum=${requestScope.vo.boardNum}'/>";
            }
        }
</script>
<c:choose>
        <c:when test="${requestScope.updateResult==true}">
            <script >alert("수정성공");</script>
        </c:when>
        <c:when test="${requestScope.updateResult==false}">
  ...
  <body>
        ...
        <div class="container">
            <div class="row align-items-center justify-content-center text-center">
                ...
                <div class="col-md-8" data-aos="fade-up" data-aos-delay="400">
                    <h1 class="text-white font-weight-light text-uppercase font-weight-bold">Notice</h1>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-7 mb-5" id="readtable">
            <table>
		        <tr>
		            <th>번호</th>
		            <th>작성자</th>
			    ...
			    <tr>
			        <th>내용</th>
			        <td colspan="3"><textarea rows="5" class="form-control" readonly="readonly">${vo.content}</textarea></td>
			    </tr>
            </table>
            <br>
            <div class="row form-group">
                    <div class="col-md-6">
                        <a href="<c:url value='/board/getBoardList'/>"><input type="button" value="목록" class="btn btn-primary py-2 px-4 text-white"></a>
	                        <c:if test="${sessionScope.userId == requestScope.vo.userId}">
	                            <a><input type="button" value="수정" onclick ="return updateConfirm()" class="btn btn-primary py-2 px-4 text-white"></a>
	                            <a><input type="button" value="삭제" onclick ="return delConfirm()" class="btn btn-primary py-2 px-4 text-white"></a>
	                        </c:if>
                    </div>
                </div>
